...
class Start
{
    ...
    function __construct()
    {
        ...
        $this->argv = $argv;
        $this->opt();
        $this->workerNums = isset($this->opt['worker']) ? (int) $this->opt['worker'] : $this->workerNums;
    ...
    public function stop()
    {
        ...
        if($res == 'y') {
            if (\Sys::$obj->os->kill($this->server_pid, SIGTERM)) {
    ...
    public function add()
    {
        ...
        if ($this->workerNums) {
            for ($i=0; $i < $this->workerNums; $i++) {
                \Sys::setProcessName('event_swoole_worker');
                if (\Sys::$obj->os->kill($this->server_pid, SIGUSR1)) {
                    echo ($i+1)." | ";
                }
                usleep(10000);
            }
    ...
    public function del()
    {
        ...
        if ($this->workerNums) {
            for ($i=0; $i < $this->workerNums; $i++) {
                if(\Sys::$obj->os->kill((int)$this->server_pid, SIGUSR2)) {
                    echo ($i+1)." | ";
                }
                usleep(10000);
            }
    ...
    public function reload()
    {
        ...
        if($this->stop()){
            $this->echo_cli('请稍后...');
            while (\Sys::$obj->os->kill($this->server_pid, 0)) {
                echo '>';
                usleep(10000);
            }
            echo "\n";
    ...
    {
        if ($this->server_pid && \Sys::$obj->os->kill($this->server_pid, 0)) {
            return true;
        }else {
            return false;
        }
